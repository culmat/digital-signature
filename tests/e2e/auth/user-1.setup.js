...
setup('authenticate user', async ({ page }) => {
  ...
  if (validator.storageStateExists(authFile)) {
    ...
    console.log(`[User ${USER_INDEX}] Session invalid: ${validation.error}`);
    validator.removeInvalidSession(authFile);
  }

  console.log(`[User ${USER_INDEX}] No valid session found, authenticating...`);
  console.log(`[User ${USER_INDEX}] Email: ${user.email}, Name: ${user.name}`);
  ...
  // Wait for possible redirects and page loads after login
  await page.waitForTimeout(3000);

  // Check that the page source contains the username (email)
